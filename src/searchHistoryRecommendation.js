export default function SearchHistoryBasedRecommendContainer({
  searchHistoryRecommendation,
}) {
  return (
    <div>
      <h2 className="RecentSearchRecommendation-heading">
        Based on your recent Searches
      </h2>
      <>
        <div className="searchHistoryBasedRecommendContainer">
          {searchHistoryRecommendation.map((book, index) => (
            <SearchHistoryBasedRecommend book={book} key={index} />
          ))}
        </div>
      </>
    </div>
...
function SearchHistoryBasedRecommend({ book }) {
  return (
    <div>
