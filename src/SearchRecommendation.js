export default function SearchTop5Container({ SearchRecommendations }) {
  return (
    <div>
      <>
        <h2 className="Top5-heading"> Most searched Books</h2>
        <div className="SearchRecommendationTop5">
          {SearchRecommendations.map((book, index) => (
            <SearchTop5 book={book} key={index} />
          ))}
        </div>
      </>
    </div>
